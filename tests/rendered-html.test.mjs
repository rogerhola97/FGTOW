...
test("contains the complete FG TOW commercial experience", async () => {
  const [home, catalog, form, api, schema, configuratorPage, configurator, quoteApi, quoteCatalog, quoteSchema] = await Promise.all([
    ...
    readFile(new URL("../app/cotizador/page.tsx", import.meta.url), "utf8"),
  ...
  assert.match(catalog, /model: "Food 2×3"/);
  ...
  assert.match(configuratorPage, /TrailerConfigurator/);
