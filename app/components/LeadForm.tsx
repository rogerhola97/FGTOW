...
export function LeadForm() {
  ...
  return (
    <form className="lead-form" onSubmit={submit}>
      ...
      <div className="form-row form-row-3"><label>Correo electrónico<input name="email" type="email" autoComplete="email" /></label><label>Ciudad<input name="city" required defaultValue="Monterrey, N.L." /></label><label>Estado<select name="state" required defaultValue={DEFAULT_STATE} autoComplete="address-level1">{MEXICAN_STATES.map((stateName) => <option key={stateName}>{stateName}</option>)}</select></label></div>
      <div className="form-row"><label>¿Qué necesitas?<select name="productType" required defaultValue=""><option value="" disabled>Selecciona una opción</option><option>Plataforma / carga</option><option>Cuatrimoto / motocicletas</option><option>RZR / Can-Am / UTV</option><option>Food Trailer</option><option>Proyecto especial</option></select></label><label>Presupuesto estimado<select name="budget" defaultValue="Por definir"><option>Por definir</option><option>Hasta $50,000</option><option>$50,000 – $100,000</option><option>$100,000 – $200,000</option><option>Más de $200,000</option></select></label></div>
