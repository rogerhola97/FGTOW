...
// lo hace. Si el body trae basedOnQuoteId, el registro nuevo queda enlazado como la siguiente
// versión de esa cotización (misma lógica que "Guardar como nueva cotización" en el editor).
export async function POST(request: Request) {
  ...
  try {
    ...
    let parentQuoteId: number | null = null;
    let version = 1;
    const basedOnQuoteId = Number(payload.basedOnQuoteId);
    if (Number.isFinite(basedOnQuoteId) && basedOnQuoteId > 0) {
      const parent = await getQuoteById(basedOnQuoteId);
      if (!parent) return Response.json({ error: "La cotización de origen ya no existe." }, { status: 400 });
      parentQuoteId = parent.id;
      version = parent.version + 1;
    }

    const result = await insertQuoteVendor({
      quote_number: quoteFolio(),
      name,
      phone,
      email,
      city,
    ...
    return Response.json({ ok: true, id: result.row.id, quoteNumber: result.row.quote_number }, { status: 201 });
  } catch (error) {
    console.error("Error al crear una cotización desde el panel de vendedor:", error);
    return Response.json({ error: "No fue posible guardar la cotización. Intenta de nuevo." }, { status: 500 });
  }
}
