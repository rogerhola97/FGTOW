import { PlacedEquipment, TRAILER_PRESETS, calculateQuote, getEquipment, money, validateLayout } from "../../lib/quoteCatalog";
...
function makePlanSvg(presetId: string, items: PlacedEquipment[]) {
...
function makeEmailHtml(args: { folio: string; name: string; phone: string; email: string; city: string; notes: string; presetId: string; items: PlacedEquipment[]; includeIva: boolean }) {
...
export async function POST(request: Request) {
  try {
    ...
    const layoutErrors = validateLayout(quote.preset, items);
    ...
    const planSvg = makePlanSvg(presetId, items);
      ...
      try {
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: [toEmail],
            reply_to: email,
            subject: `${folio} · Food trailer ${(quote.preset.widthCm / 100).toFixed(2)} × ${(quote.preset.lengthCm / 100).toFixed(2)} m · ${name}`,
            html: makeEmailHtml({ folio, name, phone, email, city, notes, presetId, items, includeIva }),
