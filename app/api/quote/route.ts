import { DOOR_MAX_WIDTH_CM, DOOR_MIN_WIDTH_CM, DoorConfig, MODEL_META, PlacedEquipment, WALL_LABEL, axleLabel, calculateQuote, defaultDoor, getEquipment, isValidPresetId, money, validateLayout } from "../../lib/quoteCatalog";
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !phone || !email || !city || !consent || !isValidPresetId(presetId) || rawItems.length !== items.length || items.length === 0) {
